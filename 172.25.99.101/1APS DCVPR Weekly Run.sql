...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'1APS_DCVPR_Weekly Run', 
		@enabled=0, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'RUN DCVPR', 
		@step_id=1, 
...
set @str1 = cast(FORMAT(GETDATE() , ''yyyyMMdd_HHmmss'') as varchar)
	...
	set @Sql = ''SQLCMD -S ONELOOK-DB-1 -i "W:\work\scripts\BMS\OneLookDataProcess\DCVPR\WeeklyRun\WeeklyRun.sql" -o "W:\work\scripts\BMS\OneLookDataProcess\CVPR\WeeklyRun\Logs\DCVPR_WeeklyRun_''+@str1+''.txt"''
