...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'1APS_Heme_CART_SITE_copy', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Copy CART SIte on PA 102', 
		@step_id=1, 
...
set @str1 = cast(FORMAT(GETDATE() , ''yyyyMMdd_HHmmss'') as varchar)
	...
	set @Sql = ''SQLCMD -S ONELOOK-DB-2 -d ''+@dbname+'' -i "W:\Work\Scripts\HEME3.0\CAR_T_SiteUpdate\Copy_CAR_T_SiteUpdate_Production.sql" -o "W:\Work\Scripts\HEME3.0\CAR_T_SiteUpdate\Logs\Copy_CAR_T_SiteUpdate_Production_''+@dbname+''_prod_''+@str1+''.txt"''
	EXEC master.sys.xp_cmdshell @Sql', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'run copy of carton pa102', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=62, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'6739193f-77a7-4ab2-9321-34fe8ae9c08d'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
