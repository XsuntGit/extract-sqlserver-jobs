...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'RUN NEURO WEEKLY', 
		@step_id=1, 
...
set @str1 = cast(FORMAT(GETDATE() , ''yyyyMMdd_HHmmss'') as varchar)
	...
	set @Sql = ''SQLCMD -S OneLook-DB-DR-1 -i "W:\work\scripts\BMS\OneLookDataProcess\NEURO\WeeklyRun\WeeklyRun.sql" -o "W:\work\scripts\BMS\OneLookDataProcess\NEURO\WeeklyRun\Logs\NEURO_WeeklyRun_''+@str1+''.txt"''
